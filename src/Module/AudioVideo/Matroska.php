...
class Matroska extends BaseHandler implements ModuleInterface
{
     ...
     * @var int
     */
    private $unuseful_elements = [ self::EBML_ID_CRC32, self::EBML_ID_VOID ];
                    ...
                    case 2: // Audio
                        ...
                        switch ( $trackarray['CodecID'] )
                        {
                            ...
                            case 'A_VORBIS':
                                ...
                                if ( !class_exists( '\\arabcoders\\getid3\\Module\\Audio\\Ogg' ) )
                                {
                    ...
                    while ( $this->getEBMLelement( $element_data, $top_element['end'] ) )
                    {
                        ...
                        switch ( $element_data['id'] )
                        {
                                ...
                                while ( $this->getEBMLelement( $track_entry, $element_data['end'] ) )
                                {
                                    switch ( $track_entry['id'] )
                                    {
                                        ...
                                        case self::EBML_ID_TRACKENTRY: //subelements: Describes a track with all elements.
                                            ...
                                            while ( $this->getEBMLelement( $subelement, $track_entry['end'], [ self::EBML_ID_VIDEO, self::EBML_ID_AUDIO, self::EBML_ID_CONTENTENCODINGS ] ) )
                                            {
                                                        ...
                                                        while ( $this->getEBMLelement( $sub_subelement, $subelement['end'] ) )
                                                        {
                                                            switch ( $sub_subelement['id'] )
                                                            {
                                                                ...
                                                                case self::EBML_ID_CONTENTENCODING:
                                                                    ...
                                                                    while ( $this->getEBMLelement( $sub_sub_subelement, $sub_subelement['end'], [ self::EBML_ID_CONTENTCOMPRESSION, self::EBML_ID_CONTENTENCRYPTION ] ) )
                                                                    {
                                ...
                                while ( $this->getEBMLelement( $subelement, $element_data['end'] ) )
                                {
                                    switch ( $subelement['id'] )
                                    {
                                        ...
                                        case self::EBML_ID_CUEPOINT:
                                            ...
                                            while ( $this->getEBMLelement( $sub_subelement, $subelement['end'], [ self::EBML_ID_CUETRACKPOSITIONS ] ) )
                                            {
                                                switch ( $sub_subelement['id'] )
                                                {
                                ...
                                while ( $this->getEBMLelement( $subelement, $element_data['end'] ) )
                                {
                                    switch ( $subelement['id'] )
                                    {
                                        ...
                                        case self::EBML_ID_ATTACHEDFILE:
                                            ...
                                            while ( $this->getEBMLelement( $sub_subelement, $subelement['end'], [ self::EBML_ID_FILEDATA ] ) )
                                            {
                                                switch ( $sub_subelement['id'] )
                                                {
                                            ...
                                            if ( !empty( $attachedfile_entry['FileData'] ) && !empty( $attachedfile_entry['FileMimeType'] ) && preg_match( '#^image/#i', $attachedfile_entry['FileMimeType'] ) )
                                            {
                                                if ( $this->getid3->option_save_attachments === GetId3Core::ATTACHMENTS_INLINE )
                                                {
                                                    ...
                                                    $info['matroska']['comments']['picture'][] = [ 'data' => $attachedfile_entry['data'], 'image_mime' => $attachedfile_entry['image_mime'], 'filename' => $attachedfile_entry['FileName'] ];
                                ...
                                while ( $this->getEBMLelement( $subelement, $element_data['end'] ) )
                                {
                                    switch ( $subelement['id'] )
                                    {
                                        ...
                                        case self::EBML_ID_EDITIONENTRY:
                                            ...
                                            while ( $this->getEBMLelement( $sub_subelement, $subelement['end'], [ self::EBML_ID_CHAPTERATOM ] ) )
                                            {
                                                switch ( $sub_subelement['id'] )
                                                {
                                                    ...
                                                    case self::EBML_ID_CHAPTERATOM:
                                                        ...
                                                        while ( $this->getEBMLelement( $sub_sub_subelement, $sub_subelement['end'], [ self::EBML_ID_CHAPTERTRACK, self::EBML_ID_CHAPTERDISPLAY ] ) )
                                                        {
                                                            switch ( $sub_sub_subelement['id'] )
                                                            {
                            ...
                            case self::EBML_ID_CLUSTER: // The lower level element containing the (monolithic) Block structure.
                                ...
                                while ( $this->getEBMLelement( $subelement, $element_data['end'], [ self::EBML_ID_CLUSTERSILENTTRACKS, self::EBML_ID_CLUSTERBLOCKGROUP, self::EBML_ID_CLUSTERSIMPLEBLOCK ] ) )
                                {
                                    switch ( $subelement['id'] )
                                    {
                                        ...
                                        case self::EBML_ID_CLUSTERBLOCKGROUP:
                                            ...
                                            while ( $this->getEBMLelement( $sub_subelement, $subelement['end'], [ self::EBML_ID_CLUSTERBLOCK ] ) )
                                            {
                                                switch ( $sub_subelement['id'] )
                                                {
    ...
    private function HandleEMBLSimpleTag( $parent_end )
    {
        ...
        while ( $this->getEBMLelement( $element, $parent_end, [ self::EBML_ID_SIMPLETAG ] ) )
        {
            switch ( $element['id'] )
            {
    ...
    {
        static $MatroskaTargetTypeValue = [];
